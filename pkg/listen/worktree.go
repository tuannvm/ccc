...
func HandleWorktreeCommand(cfg *configpkg.Config, chatID, threadID int64, text string, worktreeAutoGenerate string) {
	...
	if len(parts) == 0 && threadID > 0 {
		...
		worktreeMsg := fmt.Sprintf("%s started\nbase: %s\nworktree: %s\n%s\n\nWorktree name was auto-generated by Claude", worktreeSessionName, baseSessionName, worktreeName, providerSummary(cfg, cfg.Sessions[worktreeSessionName]))
	...
	worktreeMsg := fmt.Sprintf("%s started\nbase: %s\nworktree: %s\n%s", worktreeSessionName, baseSessionName, worktreeName, providerSummary(cfg, cfg.Sessions[worktreeSessionName]))
	if autoGenerated {
