...
// relayTransfers holds all active transfers (protected by mutex)
...
func HandleSendFile(filePath string) error {
	...
	if fileSize < maxTelegramFileSize {
		...
		return telegram.SendFile(cfg, cfg.GroupID, topicID, filePath, "")
	}
	...
	if err := telegram.SendMessage(cfg, cfg.GroupID, topicID, msg); err != nil {
		return err
	}
...
// StreamFileToRelay manages the file streaming process after registration
