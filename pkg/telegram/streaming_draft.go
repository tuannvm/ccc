...
func SendDraftMessage(cfg *config.Config, chatID int64, threadID int64, text string) error {
	...
	result, err := TelegramAPI(cfg, "sendMessageDraft", params)
	if err != nil {
...
func StreamResponse(cfg *config.Config, chatID int64, threadID int64, chunks <-chan StreamChunk, done <-chan bool) (string, error) {
		...
		case chunk, ok := <-chunks:
			if !ok {
				...
				if finalText != lastText {
					SendDraftMessage(cfg, chatID, threadID, finalText)
				}
			...
			if shouldUpdate {
				...
				if currentText != lastText {
					...
					if err := SendDraftMessage(cfg, chatID, threadID, currentText); err != nil {
...
func FinalizeStream(cfg *config.Config, chatID int64, threadID int64, text string, parseMode string) (int64, error) {
	...
	return SendMessageWithMode(cfg, chatID, threadID, text, parseMode)
}
...
func StreamAndFinalize(cfg *config.Config, chatID int64, threadID int64, text <-chan string, parseMode string) (int64, error) {
	...
	// Stream the response
	finalText, err := StreamResponse(cfg, chatID, threadID, chunks, make(chan bool))
	if err != nil {
		return 0, err
	}

	// Finalize with permanent message
	return FinalizeStream(cfg, chatID, threadID, finalText, parseMode)
}
...
func SendStreamingMessage(cfg *config.Config, chatID int64, threadID int64, text string, enableStreaming bool) (int64, error) {
	if !enableStreaming {
		// Fallback to standard message sending
		return SendMessageGetID(cfg, chatID, threadID, text)
	}
	...
	if len(text) <= chunkSize {
		...
		chunks <- text
		close(chunks)
		return StreamAndFinalize(cfg, chatID, threadID, chunks, "Markdown")
	}
